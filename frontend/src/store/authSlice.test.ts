import authReducer, { setCredentials, setAccessToken, clearCredentials } from './authSlice';
...
describe('authSlice', () => {
  ...
  it('setCredentials stores user, token and marks authenticated', () => {
    const state = authReducer(undefined, setCredentials({ user: mockUser, accessToken: 'at', refreshToken: 'rt' }));
  ...
  it('setAccessToken updates only the token field', () => {
    const after = authReducer(
      authReducer(undefined, setCredentials({ user: mockUser, accessToken: 'at', refreshToken: 'rt' })),
  ...
  it('clearCredentials resets all auth state and localStorage', () => {
    const loggedIn = authReducer(undefined, setCredentials({ user: mockUser, accessToken: 'at', refreshToken: 'rt' }));
