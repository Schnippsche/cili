...
import type { AiSummaryDto, MetadataDto, ProcessingJobDto, ResourceDto, SubtitleTrackDto, UpdateMetadataRequest } from '../types/api';
...
export function getDownloadUrl(id: number): string { return `${BASE}/api/resources/${id}/download${tokenParam()}`; }
export function getStreamUrl(id: number): string   { return `${BASE}/api/resources/${id}/stream${tokenParam()}`; }
export function getPreviewUrl(id: number): string  { return `${BASE}/api/resources/${id}/preview${tokenParam()}`; }
...
export async function analyzeResource(id: number, languageCode: string, force = false): Promise<{ jobId: number }> {
  const { data } = await axiosClient.post<{ jobId: number }>(
