  ...
  return (
    <AppShell>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        ...
        <FolderActions parentId={undefined} canWrite={isAdmin} contextFolder={contextFolder} menuAnchor={menuAnchor}
      ...
      {rootFolders.length > 0 && (
        <>
          ...
          <FolderGrid folders={rootFolders} showMenu={isAdmin} onMenuOpen={(e, f) => { setMenuAnchor(e.currentTarget as HTMLElement); setContextFolder(f); }} />
          <Divider sx={{ my: 3 }} />
        </>
      )}
      ...
      {favorites.length > 0 && (
        <>
          <Typography variant="subtitle2" color="text.secondary" gutterBottom>Favorisierte Dateien</Typography>
          <ResourceGrid resources={favorites} folderId={0} />
        </>
      )}

      {rootFolders.length === 0 && favorites.length === 0 && (
        <Typography color="text.secondary">Noch keine Ordner. Klicke auf „Neuer Ordner" um zu beginnen.</Typography>
      )}
