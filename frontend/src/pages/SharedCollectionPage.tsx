...
import {
  Alert, Box, Card, CardActionArea, CardContent, CardMedia,
...
import type { SharedResourceItem, PublicTestimonialDto } from '../types/api';
  ...
  if (isVideo || isAudio) {
    return (
      <Box sx={{ bgcolor: 'black', p: 2 }}>
        <VideoPlayer src={src} mimeType={resource.mimeType} audioOnly={isAudio} subtitles={subtitles} />
      </Box>
    );
  }
  if (isImage) {
    ...
    <Card variant="outlined" sx={{ mb: 2 }}>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 0.5, mt: 0.25 }}>
          ...
          {tagList.map(tag => (
            <Box key={tag} component="span" sx={{ bgcolor: 'info.light', borderRadius: 1, px: 0.75, py: 0.25, fontSize: '0.7rem' }}>
              {tag}
            </Box>
          ))}
        </Box>
