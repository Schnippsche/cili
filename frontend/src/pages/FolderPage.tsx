...
import {
  useActiveClipJobs,
  useCreateMediaClip,
  useResourcesByFolder,
  useSubtitleTracks
} from '../hooks/useResources';
import {getPreviewUrl, getStreamUrl, getSubtitleUrl, reorderResources} from '../api/resources';
...
export default function FolderPage() {
  ...
  const [clipSnack, setClipSnack] = useState<{
    open: boolean;
      ...
      <AppShell>
        ...
        {subFolders.length > 0 && (
            <>
              ...
              {viewMode === 'grid'
                  ?
                  <FolderGrid folders={subFolders} showMenu={canFolderMenu} onMenuOpen={(e, f) => {
                    setMenuAnchor(e.currentTarget as HTMLElement);
                    setContextFolder(f);
                  }}/>
                  :
                  <FolderList folders={subFolders} showMenu={canFolderMenu} onMenuOpen={(e, f) => {
                    setMenuAnchor(e.currentTarget as HTMLElement);
                    setContextFolder(f);
                  }}/>
              }
        ...
        {id != null && (
            <>
              <Typography variant="subtitle2" color="text.secondary"
                          gutterBottom>Dateien</Typography>
              {viewMode === 'grid'
                  ? <ResourceGrid resources={resources} folderId={id} canDownload={canDownload}
                                  canEdit={canEdit} canDelete={canDelete} canReorder={canReorder}
                                  canMove={canMove} canShare={canShare} onReorder={handleReorder}/>
                  : <ResourceList resources={resources} folderId={id} canDownload={canDownload}
                                  canEdit={canEdit} canDelete={canDelete} canReorder={canReorder}
                                  canMove={canMove} canShare={canShare} onReorder={handleReorder}/>
              }
        ...
        {viewId != null && viewType !== 'image' && (
            <Dialog open fullWidth maxWidth="lg" onClose={() => setSearchParams({})}>
              <DialogTitle sx={{display: 'flex', alignItems: 'center', pb: 0}}>
                ...
                <Link component="button" variant="body2" onClick={() => setSearchParams({})}>Ansicht
                  schließen</Link>
              </DialogTitle>
              <DialogContent>
                    ...
                    <>
                      ...
                      {canUpload && (
                          <Box sx={{mt: 1, display: 'flex', justifyContent: 'flex-end'}}>
                            <Button size="small" startIcon={<ContentCutIcon/>}
                                    ...
                                    {id: viewId, startMs, endMs, title},
                                    {
                                      ...
                                      onError: () => {
                                        setClipSnack({
                                          open: true,
                                          severity: 'error',
                                          msg: 'Clip konnte nicht erstellt werden.'
                                        });
                                      },
                                    }
                                );
        ...
        >
          <Alert severity={clipSnack.severity}
                 onClose={() => setClipSnack(s => ({...s, open: false}))}>
            {clipSnack.msg}
          </Alert>
        </Snackbar>
