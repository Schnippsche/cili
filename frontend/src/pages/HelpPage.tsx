...
import {
  Box, Container, Divider, Paper, Table, TableBody,
    ...
    <AppShell>
      <Container maxWidth="md" sx={{ py: 4 }}>
        <ReactMarkdown
          ...
          components={{
            h1: ({ children }) => (
              <Typography id={headingId(children)} variant="h4" fontWeight="bold" gutterBottom sx={{ mt: 2 }}>
                {children}
              </Typography>
            ),
            h2: ({ children }) => (
              <Typography id={headingId(children)} variant="h5" fontWeight="bold" gutterBottom sx={{ mt: 4, mb: 1 }}>
                {children}
              </Typography>
            ),
            h3: ({ children }) => (
              <Typography id={headingId(children)} variant="h6" fontWeight="bold" gutterBottom sx={{ mt: 3 }}>
                {children}
              </Typography>
            ),
            h4: ({ children }) => (
              <Typography id={headingId(children)} variant="subtitle1" fontWeight="bold" gutterBottom sx={{ mt: 2 }}>
                {children}
              </Typography>
            ),
            p: ({ children }) => (
