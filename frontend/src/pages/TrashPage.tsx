...
import {
  ...
  DialogTitle, IconButton, List, ListItem, ListItemText, Snackbar, Tooltip, Typography,
} from '@mui/material';
import DeleteForeverOutlinedIcon from '@mui/icons-material/DeleteForeverOutlined';
import RestoreFromTrashOutlinedIcon from '@mui/icons-material/RestoreFromTrashOutlined';
import AppShell from '../components/layout/AppShell';
import { useTrash, useRestoreFolder, usePurgeFolder } from '../hooks/useFolders';
import type { FolderDto } from '../types/api';

function formatDate(iso: string | null): string {
  if (!iso) return '';
  const d = new Date(iso);
  return ` · gelöscht am ${d.toLocaleDateString('de-DE')} ${d.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}`;
}
...
export default function TrashPage() {
  ...
  function handlePurgeConfirm() {
    ...
    purge.mutate(pendingPurge.id, {
      ...
      onError:   () => { setPendingPurge(null); setErrorMessage('Löschen fehlgeschlagen.'); },
    });
  }
  ...
  return (
    <AppShell>
      ...
      {folders && folders.length > 0 && (
        <List>
          {folders.map(folder => (
            ...
            >
              <ListItemText
                primary={<Box component="span" sx={{ fontWeight: 'bold' }}>{folder.name}</Box>}
      ...
      <Dialog open={pendingPurge !== null} onClose={() => setPendingPurge(null)}>
        ...
        <DialogContent>
          Ordner „{pendingPurge?.name}" und alle enthaltenen Dateien werden unwiderruflich gelöscht.
        </DialogContent>
        <DialogActions>
