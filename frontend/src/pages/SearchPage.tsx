  ...
  return (
      <AppShell>
        ...
        <Alert severity="info" sx={{mb: 2}}>
          ...
          Für eine gezielte Suche nur in Erfahrungsberichten steht die separate
          Erfahrungsberichte-Seite zur Verfügung.
        </Alert>
        <Box sx={{mb: 3}}>
          <SearchBar value={query} onChange={setQuery}/>
          <Typography variant="caption" color="text.secondary" sx={{mt: 0.5, display: 'block'}}>
            Mehrere Begriffe werden mit UND verknüpft – alle Begriffe müssen enthalten sein.
            Präfixsuche: „erfahr" findet auch „Erfahrung".
          </Typography>
        </Box>
        <Box>
                  ...
                  {results.hits.map(hit => (
                      ...
                      >
                        <ListItemButton sx={{pr: 6}} onClick={() => {
                          ...
                          if (type === 'text') {
                            navigate(`/resources/${hit.resourceId}/edit`);
                            return;
                          }
                          const target = type
                                  ...
                                  <>
                                    ...
                                    {hit.snippets.map((s, i) => {
                                      ...
                                      return (
                                          <span
                                              key={i}
                                              onClick={snippetTarget ? (e) => {
                                                e.stopPropagation();
                                                navigate(snippetTarget);
                                              } : undefined}
                                              style={{
                        ...
                        {results.testimonialHits.map((t: TestimonialSearchHitDto) => (
                            ...
                            >
                              <ListItemButton sx={{pr: 6}}
                                              onClick={() => navigate(`/testimonials?id=${t.id}`)}>
                                <ListItemText
                                    primary={
                                      <Box sx={{
                                        display: 'flex',
                                        ...
                                        <Chip label="Testimonial" size="small" variant="outlined"
                                              color="secondary"/>
                                        {t.human &&
                                            <Chip icon={<PersonIcon/>} label="Mensch" size="small"
                                                  color="primary"/>}
                                        {t.animal &&
                                            <Chip icon={<PetsIcon/>} label="Tier" size="small"
                                                  color="success"/>}
