...
import {
  ...
  Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControlLabel, Grid2,
  IconButton, List, ListItemButton, ListItemText, TextField, Tooltip, Typography
} from '@mui/material';
...
import {
  useCollections, useCollectionTemplates, useCreateCollection, useCreateCollectionFromTemplate,
  useCopyCollection, useDeleteCollection, useRenameCollection
...
export default function CollectionsPage() {
  ...
  const handleCopy = () => {
    ...
    copyMutation.mutate(
      ...
      { onSuccess: () => { setCopyTarget(null); setCopyName(''); } }
    );
  };
    ...
    <AppShell>
      <Box sx={{ p: 3 }}>
        ...
        <Alert severity="info" sx={{ mb: 3 }}>
          Sammlungen ermöglichen es, Dateien aus verschiedenen Ordnern thematisch zu gruppieren — ohne sie zu verschieben.
          Zusätzlich können Erfahrungsberichte hinzugefügt werden, um Inhalte mit persönlichen Eindrücken zu ergänzen.
          Über einen Freigabe-Link lässt sich eine Sammlung mit Externen teilen, ohne dass diese einen Login benötigen.
          Nur du siehst deine eigenen Sammlungen. Beim Anlegen kannst du eine vom Administrator bereitgestellte Vorlage verwenden,
          ...
          {collections.map(c => (
            <Grid2 key={c.id} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
              <Card variant="outlined">
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2, minHeight: 32 }}>
                  <Box>
        ...
        {/* Dialog: Kopieren */}
        <Dialog open={!!copyTarget} onClose={() => { setCopyTarget(null); setCopyName(''); }} maxWidth="xs" fullWidth>
          ...
          <DialogActions>
            <Button onClick={() => { setCopyTarget(null); setCopyName(''); }}>Abbrechen</Button>
