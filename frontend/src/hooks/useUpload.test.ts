...
describe('useUpload', () => {
  ...
  describe('audio normalization polling', () => {
    ...
    it('polls the uploaded resource until it is normalized to audio/mpeg, then stops', async () => {
      mockedInit.mockResolvedValueOnce({ jobId: 'j2', chunksTotal: 1, chunksReceived: 0, status: 'INITIATED' });
      mockedChunk.mockResolvedValueOnce({ jobId: 'j2', chunksTotal: 1, chunksReceived: 1, status: 'IN_PROGRESS' });
    ...
    it('does not poll for non-audio uploads', async () => {
      mockedInit.mockResolvedValueOnce({ jobId: 'j3', chunksTotal: 1, chunksReceived: 0, status: 'INITIATED' });
      mockedChunk.mockResolvedValueOnce({ jobId: 'j3', chunksTotal: 1, chunksReceived: 1, status: 'IN_PROGRESS' });
