...
export function useAuth() {
  ...
  useEffect(() => {
    const onLogout = () => { dispatch(clearCredentials()); qc.clear(); navigate('/login'); };
  ...
  const login = useCallback(async (req: LoginRequest) => {
    ...
    qc.clear();
    dispatch(setCredentials({ user: resp.user, accessToken: resp.accessToken, refreshToken: resp.refreshToken }));
    navigate('/');
  }, [dispatch, navigate, qc]);
