...
describe('useBulkImportUpload', () => {
  ...
  test('start: uploads all PENDING items and marks them DONE', async () => {
    vi.mocked(bulkImportApi.createBulkImport).mockResolvedValue({
      jobId: 'job1',
      items: [
        { id: 1, relativePath: 'video1.mp4', resolvedFolderId: 10, status: 'PENDING', skipReason: null, errorMessage: null, resourceId: null },
        { id: 2, relativePath: 'notes.exe', resolvedFolderId: null, status: 'SKIPPED', skipReason: 'Nicht unterstützt', errorMessage: null, resourceId: null },
      ],
    });
    vi.mocked(uploadApi.initUpload).mockResolvedValue({ jobId: 'up1', chunksTotal: 1, chunksReceived: 0, status: 'INITIATED' });
    vi.mocked(uploadApi.uploadChunk).mockResolvedValue({ jobId: 'up1', chunksTotal: 1, chunksReceived: 1, status: 'IN_PROGRESS' });
    ...
    expect(uploadApi.initUpload).toHaveBeenCalledWith(expect.objectContaining({ bulkImportItemId: 1, folderId: 10 }));
  });
  ...
  test('start: marks item FAILED and reports it to the backend after retries are exhausted', async () => {
    vi.mocked(bulkImportApi.createBulkImport).mockResolvedValue({
      jobId: 'job2',
      items: [
        { id: 3, relativePath: 'video1.mp4', resolvedFolderId: 10, status: 'PENDING', skipReason: null, errorMessage: null, resourceId: null },
      ],
    });
