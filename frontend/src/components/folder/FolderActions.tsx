import { Alert, AlertTitle, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, LinearProgress, Menu, MenuItem, Snackbar, TextField, Tooltip, Typography } from '@mui/material';
import { useEffect, useMemo, useState } from 'react';
import { useSelector } from 'react-redux';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import * as adminApi from '../../api/admin';
import { useCreateFolder, useFolderPermissions, useMoveFolder, useTrashFolder, useUpdateFolder } from '../../hooks/useFolders';
...
interface Props { parentId?: number; folderId?: number; canWrite: boolean; canUpload?: boolean; contextFolder: FolderDto | null; menuAnchor: HTMLElement | null; onMenuClose: () => void; }

export default function FolderActions({ parentId, folderId, canWrite, canUpload = false, contextFolder, menuAnchor, onMenuClose }: Readonly<Props>) {
  ...
  const [doneSnack, setDoneSnack] = useState<{ open: boolean; severity: 'success' | 'error'; msg: string }>(
  ...
  interface ImportProgress { phase: 'download' | 'processing' | 'uploading'; progress?: number; }
  const importProgress = useMemo<ImportProgress | null>(() => {
    if (!polledJob?.result) return null;
    try { return JSON.parse(polledJob.result) as ImportProgress; }
    catch { return null; }
  }, [polledJob?.result]);
  ...
  useEffect(() => {
    ...
    if (polledJob.status === 'DONE') {
      ...
      setActiveImportJob(null);
      setDoneSnack({ open: true, severity: 'error', msg: polledJob.errorMessage ?? 'Download fehlgeschlagen.' });
    } else {
  ...
  const handleRename = async () => {
    ...
    setRenameOpen(false); setName(''); onMenuClose();
  };
  ...
  return (
    <>
      ...
      {canWrite && <Button variant="outlined" size="small" onClick={() => setCreateOpen(true)}>Neuer Ordner</Button>}

      <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={onMenuClose}>
        {canRenameCtx && <MenuItem onClick={() => { setActionFolder(contextFolder); setName(contextFolder?.name ?? ''); setRenameOpen(true); onMenuClose(); }}>Umbenennen</MenuItem>}
        {canDeleteCtx && (
          <MenuItem onClick={() => {
        ...
        {canDeleteCtx && <MenuItem sx={{ color: 'error.main' }} onClick={() => { setActionFolder(contextFolder); setTrashOpen(true); onMenuClose(); }}>In Papierkorb</MenuItem>}
      </Menu>

      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="xs" fullWidth>
        <DialogTitle>Neuer Ordner</DialogTitle>
        <DialogContent>
          <TextField autoFocus fullWidth label="Ordnername" value={name} onChange={e => setName(e.target.value)} sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={renameOpen} onClose={() => setRenameOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <TextField autoFocus fullWidth label="Neuer Name" value={name} onChange={e => setName(e.target.value)} sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setRenameOpen(false)}>Abbrechen</Button>
          <Button variant="contained" onClick={handleRename} disabled={!name.trim()}>Umbenennen</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={trashOpen} onClose={() => setTrashOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          Der Ordner <strong>{actionFolder?.name}</strong> und alle enthaltenen Inhalte werden in den Papierkorb verschoben.
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setTrashOpen(false)}>Abbrechen</Button>
          <Button variant="contained" color="error" onClick={() => { if (actionFolder) { trashFolder.mutate(actionFolder.id); setTrashOpen(false); } }}>
            In Papierkorb
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Snackbar open={isImportRunning} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
        <Alert severity="info" icon={<CircularProgress size={16} />} sx={{ minWidth: 320 }}>
          ...
          {importProgress?.phase === 'download' && importProgress.progress != null ? (
            <Box>
              <Typography variant="body2" sx={{ mb: 0.5 }}>Download: {importProgress.progress}%</Typography>
              <LinearProgress variant="determinate" value={importProgress.progress} sx={{ borderRadius: 1 }} />
            </Box>
      ...
      >
        <Alert severity={doneSnack.severity} onClose={() => setDoneSnack(s => ({ ...s, open: false }))}>
          {doneSnack.msg}
        </Alert>
      </Snackbar>
    </>
