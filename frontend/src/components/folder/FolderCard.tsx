...
interface Props { folder: FolderDto; onMenuOpen: (e: MouseEvent, f: FolderDto) => void; showMenu?: boolean; }
    ...
    <Card variant="outlined" sx={{ position: 'relative', '&:hover': { boxShadow: 2 } }}>
      <CardActionArea onClick={() => navigate(`/folders/${folder.id}`)}>
        <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
          ...
          <Tooltip title={folder.name} placement="bottom" enterDelay={300} disableHoverListener={!isTruncated}>
            <Typography
              ref={textRef}
              variant="body2"
              align="center"
      ...
      {showMenu && (
        <Tooltip title="Optionen">
          <IconButton size="small" onClick={(e) => { e.stopPropagation(); onMenuOpen(e, folder); }}
