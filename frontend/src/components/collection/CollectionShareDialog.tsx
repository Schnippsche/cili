...
import {
  createCollectionShare, getCollectionShare, revokeCollectionShare,
...
export default function CollectionShareDialog({ collection, onClose }: Readonly<Props>) {
  ...
  const expiresAt = shareToken
    ? new Date(shareToken.expiresAt).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' })
    : null;
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        {isError && (
          <Alert severity="error">Fehler beim Laden des Links. Bitte Dialog schließen und erneut öffnen.</Alert>
        )}

        {!isLoading && !isError && !shareToken && (
          <Box>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Erstelle einen öffentlichen Link, mit dem Interessenten die Sammlung ohne Login einsehen können.
