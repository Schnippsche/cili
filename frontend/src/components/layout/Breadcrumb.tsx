...
export default function Breadcrumb({crumbs}: Readonly<{ crumbs: BreadcrumbItemDto[] }>) {
  ...
  return (
      <Breadcrumbs aria-label="breadcrumb" sx={{mb: 2}} maxItems={isMobile ? 2 : 8}>
        <Link component={RouterLink as ElementType} to="/" underline="hover"
              color="inherit">Home</Link>
        {ancestors.map((c) => (
            <Link key={c.id} component={RouterLink as ElementType} to={`/folders/${c.id}`}
                  underline="hover" color="inherit">{c.name}</Link>
        ))}
