      ...
      <>
        ...
        >
          ...
          <TableCell onClick={handleOpen}>
            <Typography variant="body2"
                        color="text.secondary">{formatBytes(resource.size)}</Typography>
          </TableCell>
          <TableCell onClick={handleOpen}>
        ...
        <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}
          ...
          {canDownload && <MenuItem onClick={() => {
            setAnchor(null);
            globalThis.open(getDownloadUrl(id), '_blank');
            ...
            <TableBody>
              {resources.map(r => (
                  <ResourceRow key={r.id} resource={r} folderId={folderId}
                               canDownload={canDownload} canEdit={canEdit} canDelete={canDelete}
                               canReorder={canReorder} canMove={canMove} canShare={canShare}/>
              ))}
            </TableBody>
          </Table>
        </SortableContext>
