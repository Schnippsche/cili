...
import { useSubtitleTracks, useRetranscribeResource, useActiveTranscriptionJob } from '../../hooks/useResources';
...
export default function SubtitleSection({ resourceId, readonly = false, canDownload = false }: Readonly<Props>) {
  ...
  return (
    <Box>
      ...
      {isError && <Alert severity="error" sx={{ mb: 1 }}>Untertitel konnten nicht geladen werden.</Alert>}
        ...
        <Box key={track.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
          ...
          {canDownload && (
            <>
              ...
              <Menu
                ...
                onClose={closeMenu}
              >
                <MenuItem onClick={() => downloadTrack(track, 'vtt')}>Untertitel als VTT</MenuItem>
                <MenuItem onClick={() => downloadTrack(track, 'srt')}>Untertitel als SRT</MenuItem>
                <MenuItem onClick={() => downloadTrack(track, 'txt')}>Untertitel als Text</MenuItem>
              </Menu>
            </>
          )}
          ...
          {!readonly && (
            <Button
              ...
              onClick={() => { setDeleteError(null); setDeletingId(track.id); }}
            >
              Löschen
            </Button>
          )}
      ...
      {!readonly && (
        <Box sx={{ mt: 1, mb: 1 }}>
          <Button
            size="small"
            startIcon={transcriptionRunning || retranscribe.isPending ? <CircularProgress size={14} /> : <RefreshIcon />}
          ...
          {retranscribe.isError && (
            <Alert severity="error" sx={{ mt: 1, py: 0 }}>Fehler beim Starten der Transkription.</Alert>
          )}
        </Box>
      )}

      <Dialog open={deletingId !== null} onClose={() => setDeletingId(null)} maxWidth="xs" fullWidth>
      ...
      <Dialog open={retranscribeOpen} onClose={() => setRetranscribeOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => { setRetranscribeOpen(false); retranscribe.mutate(resourceId); }}>
