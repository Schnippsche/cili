...
function typeInfo(mimeType: string): { label: string; color: string } {
  ...
  if (/sheet|excel|opendocument\.spreadsheet/.test(mimeType)) return {
    label: 'XLS',
    color: '#2e7d32'
  };
  if (/presentation|powerpoint|opendocument\.presentation/.test(mimeType)) return {
    label: 'PPT',
    color: '#bf360c'
  };
  ...
  return (
      ...
      >
        <Box sx={{
          ...
          <Typography
              sx={{fontSize: '0.6rem', fontWeight: 700, letterSpacing: 0.8, color, lineHeight: 1}}>
            {label}
          </Typography>
        </Box>
        <ResourceCard resource={resource} canDownload={canDownload} canEdit={canEdit}
                      canDelete={canDelete} canMove={canMove} canShare={canShare}
          ...
          <Grid2 container spacing={2} alignItems="stretch">
            {resources.map(r => (
                <Grid2 key={r.id} size={{xs: 6, sm: 4, md: 3, lg: 2}} sx={{display: 'flex'}}>
                  <SortableCard resource={r} canDownload={canDownload} canEdit={canEdit}
                                canDelete={canDelete} canReorder={canReorder} canMove={canMove}
                                canShare={canShare}
                                onRemoveFromCollection={onRemoveFromCollection}/>
                </Grid2>
            ))}
