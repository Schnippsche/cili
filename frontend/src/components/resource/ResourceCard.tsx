...
function FileName({title, originalName}: Readonly<{ title: string | null; originalName: string }>) {
  ...
  return (
      <Tooltip title={display} placement="bottom" enterDelay={300}
               disableHoverListener={!isTruncated}>
        <Box sx={{display: 'flex', overflow: 'hidden', alignItems: 'baseline'}}
             onMouseEnter={checkTruncation}>
          <Typography ref={baseRef} variant="caption" noWrap sx={{minWidth: 0}}>{base}</Typography>
          {ext && <Typography variant="caption"
                              sx={{flexShrink: 0, color: 'text.secondary'}}>{ext}</Typography>}
        </Box>
      </Tooltip>
  );
}
...
interface Props {
  ...
  canMove?: boolean;
  canShare?: boolean;
  onRemoveFromCollection?: (resourceId: number) => void;
}
  ...
  return (
      <>
        <Card variant="outlined" sx={{
          ...
          <CardActionArea onClick={handleOpen} disabled={!canOpen} sx={{
            flex: 1,
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'stretch'
            ...
            <CardContent sx={{pt: 1, pb: '8px !important', mt: 'auto'}}>
              <Box sx={{display: 'flex', alignItems: 'flex-start', gap: 0.5}}>
                <Box sx={{flex: 1, overflow: 'hidden'}}>
                  <FileName title={resource.metadata?.title ?? null}
          ...
          <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}>
            {canDownload && <MenuItem onClick={() => {
              setAnchor(null);
              globalThis.open(getDownloadUrl(resource.id), '_blank');
            }}>Herunterladen</MenuItem>}
            {canEdit && <MenuItem onClick={() => {
              setAnchor(null);
              setMetaReadonly(false);
              setMetaOpen(true);
            }}>Metadaten bearbeiten</MenuItem>}
            {!canEdit && <MenuItem onClick={() => {
              setAnchor(null);
              setMetaReadonly(true);
              setMetaOpen(true);
            }}>Metadaten ansehen</MenuItem>}
            {isMediaType && <MenuItem onClick={() => {
              setAnchor(null);
              setSubtitleOpen(true);
            }}>Untertitel</MenuItem>}
            {isMediaType && resource.hasAnalyzableSubtitles && <MenuItem onClick={() => {
              setAnchor(null);
              setAnalysisOpen(true);
            }}>KI-Zusammenfassung</MenuItem>}
            {canShare && <MenuItem onClick={() => {
              setAnchor(null);
              setShareOpen(true);
            }}>Freigabelink</MenuItem>}
            <MenuItem onClick={() => {
              setAnchor(null);
            ...
            {canMove && <MenuItem onClick={() => {
              setAnchor(null);
              setMoveOpen(true);
            }}>Verschieben</MenuItem>}
            {canDelete && <MenuItem sx={{color: 'error.main'}} onClick={() => {
              setAnchor(null);
              setDeleteOpen(true);
            }}>Löschen</MenuItem>}
            {onRemoveFromCollection && (
                <MenuItem onClick={() => {
                  setAnchor(null);
                  onRemoveFromCollection(resource.id);
                }}>
        ...
        <MetadataPanel resource={resource} open={metaOpen} onClose={() => setMetaOpen(false)}
                       readonly={metaReadonly}/>
        <SubtitlePanel resource={resource} open={subtitleOpen}
                       onClose={() => setSubtitleOpen(false)}/>
        <AnalysisPanel resource={resource} open={analysisOpen}
                       onClose={() => setAnalysisOpen(false)}/>
        <SharePanel resource={resource} open={shareOpen} onClose={() => setShareOpen(false)}/>
        <AddToCollectionDialog open={collectionOpen} itemId={resource.id} itemType="resource"
        ...
        <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} maxWidth="xs" fullWidth>
          ...
          <DialogContent>
            <strong>{resource.metadata?.title || resource.originalName}</strong> wird unwiderruflich
            gelöscht.
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setDeleteOpen(false)}>Abbrechen</Button>
            <Button variant="contained" color="error" onClick={() => {
              deleteRes.mutate(resource.id);
              setDeleteOpen(false);
            }}>Löschen</Button>
          </DialogActions>
        </Dialog>
