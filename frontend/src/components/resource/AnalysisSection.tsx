import { Alert, Box, Button, CircularProgress, Tooltip, ToggleButton, ToggleButtonGroup, Typography } from '@mui/material';
...
import { useSubtitleTracks, useAiSummaries, useAnalyzeResource, useActiveAnalysisJobs } from '../../hooks/useResources';
...
export default function AnalysisSection({ resourceId }: Readonly<Props>) {
  ...
  useEffect(() => { analyze.reset(); }, [resourceId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const jobForLang = (lang: string | null) => activeJobs.find(j => {
    try { return JSON.parse(j.result ?? '{}').languageCode === lang; }
    catch { return false; }
  });
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      {analyzableTracks.length > 1 && (
        <ToggleButtonGroup
          value={selectedLang}
          exclusive
          onChange={(_, v) => { if (v) setSelectedLang(v); }}
          size="small"
        >
              ...
              <Tooltip key={t.languageCode} title={languageLabel(t.languageCode)} arrow>
                <ToggleButton value={t.languageCode} sx={{ gap: 0.5 }}>
                  ...
                  {jobRunning
                    ...
                    : hasSummary && <CheckCircleOutlineIcon sx={{ fontSize: 14, color: 'success.main' }} />}
                </ToggleButton>
              </Tooltip>
            );
          })}
