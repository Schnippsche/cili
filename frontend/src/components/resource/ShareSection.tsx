...
interface Props { resourceId: number; }

function formatExpiry(expiresAt: string): string {
  const exp = new Date(expiresAt);
  const now = new Date();
  const diffDays = Math.ceil((exp.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  const dateStr = exp.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' });
...
export default function ShareSection({ resourceId }: Readonly<Props>) {
  ...
  const handleCopy = async () => {
    ...
    if (!success && navigator.clipboard?.writeText) {
      try { await navigator.clipboard.writeText(shareUrl); success = true; } catch { /* ignore */ }
    }
    if (success) {
      setCopyState('copied');
    ...
    <Box>
      ...
      {!isLoading && token && shareUrl && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          {copyState === 'copied' && <Alert severity="success" sx={{ py: 0 }}>Link kopiert!</Alert>}
          {copyState === 'manual' && <Alert severity="info" sx={{ py: 0 }}>URL markiert – Strg+C zum Kopieren drücken</Alert>}
          <Tooltip title="Nach Ablauf wird der Link automatisch ungültig" placement="bottom-start">
            <Typography variant="caption" color="text.secondary"
              sx={{ display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'default', width: 'fit-content' }}>
              <AccessTimeIcon sx={{ fontSize: 13 }} />
