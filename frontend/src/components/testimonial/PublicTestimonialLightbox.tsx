...
import {
  getPublicStreamUrl,
  getPublicSubtitleTracks,
  getPublicSubtitleUrl,
  publicImageUrl
} from '../../api/publicTestimonials';
...
export default function PublicTestimonialLightbox({
                                                    images,
                                                    testimonialId,
                                                    index,
                                                    onClose,
      ...
      <Modal open onClose={onClose} role="dialog">
        <Box sx={{
          ...
          <Box sx={{
            ...
            {isVideo && (
                <VideoPlayer src={getPublicStreamUrl(testimonialId, current.id)}
                             mimeType={current.mimeType} subtitles={subtitles}/>
            )}
            {isAudio && (
                <VideoPlayer src={getPublicStreamUrl(testimonialId, current.id)} audioOnly
                             subtitles={subtitles}/>
            )}
          </Box>
