...
export default function TestimonialList({ canWrite, canDelete, highlightId }: Readonly<Props>) {
  ...
  return (
    <Box id="testimonials">
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
        {deferredQ && (data?.page.totalElements ?? 0) > 0 && (
          <Tooltip title="Aktuelles Suchergebnis als druckbaren Bericht zusammenstellen">
            <Button startIcon={<SummarizeIcon />} size="small" variant="outlined" onClick={() => setReportOpen(true)}>
              Bericht generieren
            </Button>
          </Tooltip>
        )}
        {deferredQ && (data?.content.length ?? 0) > 0 && (
          <Tooltip title="Alle Treffer des aktuellen Suchergebnisses zu einer Sammlung hinzufügen">
            <Button startIcon={<PlaylistAddIcon />} size="small" variant="outlined" onClick={() => setBulkDialogOpen(true)}>
              Alle in Sammlung
            </Button>
          </Tooltip>
        )}
      ...
      {highlightedItem && (
        <Box ref={highlightRef} sx={{ mb: 3, border: '2px solid', borderColor: 'primary.main', borderRadius: 2, p: 0.5 }}>
