...
import { List, ListItem, ListItemIcon, ListItemText, Typography, Box, Tooltip } from '@mui/material';
...
function statusIcon(status: BulkImportProgressItem['status']) {
  switch (status) {
    ...
    case 'SKIPPED': return <BlockIcon fontSize="small" color="disabled" />;
    case 'UPLOADING': return <CircularProgress size={16} />;
    default: return <ScheduleIcon fontSize="small" color="disabled" />;
  }
}
...
const BulkImportRow = memo(function BulkImportRow({ item }: Readonly<RowProps>) {
  return (
    <ListItem>
      ...
      <Tooltip title={item.skipReason ?? item.errorMessage ?? ''} disableHoverListener={!item.skipReason && !item.errorMessage}>
        <ListItemText
  ...
  return (
    <Box>
      <Typography variant="body2" color="text.secondary" gutterBottom>
        {items.length} Dateien insgesamt — {done} fertig, {skipped} übersprungen, {failed} fehlgeschlagen
      </Typography>
