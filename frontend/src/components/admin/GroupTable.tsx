  ...
  return (
      <Dialog open onClose={onClose} maxWidth="xs" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent sx={{display: 'flex', flexDirection: 'column', gap: 2, mt: 1}}>
          ...
          <TextField label="Beschreibung" value={desc} onChange={e => setDesc(e.target.value)}
                     fullWidth multiline rows={2}/>
        </DialogContent>
        <DialogActions>
          <Button onClick={onClose}>Abbrechen</Button>
          <Button variant="contained" disabled={!name || update.isPending}
      ...
      <Dialog open onClose={onClose} maxWidth="sm" fullWidth fullScreen={isMobile}>
        <DialogTitle>
          ...
          <Typography variant="body2"
                      color="text.secondary">{members.length} Mitglied(er)</Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{display: 'flex', gap: 1, mb: 2}}>
            <Autocomplete
                ...
                renderInput={params => <TextField {...params} label="User hinzufügen"
                                                  size="small"/>}
      ...
      <Dialog open onClose={onClose} maxWidth="md" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent>
          ...
          <Box sx={{display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center'}}>
            ...
            <FormControlLabel
                control={<Checkbox checked={inheritable}
                                   onChange={e => setInheritable(e.target.checked)} size="small"/>}
                label="Vererbbar"
            />
            <Button variant="contained"
                    disabled={!selFolder || selPerms.length === 0 || addEntry.isPending}
          ...
          <Typography variant="caption" color="text.secondary">
            Erlaubt Mitgliedern dieser Gruppe, eigene Sammlungen als Vorlage zu markieren
            (unabhängig vom Admin-Status).
          </Typography>
        </DialogContent>
        <DialogActions><Button onClick={onClose}>Schließen</Button></DialogActions>
      </Dialog>
...
function DeleteGroupConfirmDialog({group, onConfirm, onClose}: Readonly<{
  group: GroupDto;
  onConfirm: () => void;
  onClose: () => void
}>) {
  ...
  return (
      <Dialog open onClose={onClose} maxWidth="xs" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent>
          Soll die Gruppe <strong>{group.name}</strong> wirklich gelöscht werden? Alle
          Berechtigungen dieser Gruppe werden ebenfalls entfernt.
        </DialogContent>
        <DialogActions>
          <Button onClick={onClose}>Abbrechen</Button>
