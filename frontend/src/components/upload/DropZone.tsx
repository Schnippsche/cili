    ...
    <Box>
      <Box onDragOver={e => { e.preventDefault(); setDragging(true); }}
           ...
           sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                 border: '2px dashed', borderRadius: 2, p: 4, cursor: 'pointer',
                 borderColor: dragging ? 'primary.main' : 'divider',
                 bgcolor: dragging ? 'action.hover' : 'background.default' }}>
        <input type="file" multiple hidden onChange={e => { uploadFiles(Array.from(e.target.files ?? [])); e.target.value = ''; }} />
        <UploadFileOutlinedIcon sx={{ fontSize: 40, color: 'text.secondary', mb: 1 }} />
        <Typography variant="body2" color="text.secondary">Dateien hier ablegen oder klicken zum Hochladen</Typography>
        <Typography variant="caption" color="text.disabled" sx={{ mt: 0.5 }}>Untertitel und Transkript werden automatisch im Hintergrund erstellt — das kann einige Minuten dauern</Typography>
      </Box>
