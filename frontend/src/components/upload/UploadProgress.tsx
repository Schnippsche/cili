...
export default function UploadProgress({uploads, onClear}: Readonly<{
  uploads: UploadEntry[];
  onClear: () => void
}>) {
          ...
          {uploads.map(u => (
              <ListItem key={u.id} disableGutters
                ...
                <Box sx={{display: 'flex', alignItems: 'center', width: '100%'}}>
                  <ListItemText
                      primary={u.fileName}
                      secondary={u.status === 'uploading' ?
                          <LinearProgress variant="determinate" value={u.progress}/> : u.error}/>
                  {u.status === 'done' &&
                ...
                {u.codecWarning && (
                    <Alert severity="warning"
                           sx={{py: 0, fontSize: '0.75rem'}}>{u.codecWarning}</Alert>
                )}
                {u.subtitleFile && (
                    <Box sx={{display: 'flex', alignItems: 'center', gap: 0.75, pl: 2.5, mt: 0.25}}>
                      ...
                      <Typography variant="caption" color="text.secondary"
                                  sx={{flexGrow: 1}}>{u.subtitleFile}</Typography>
                      {u.subtitleStatus === 'pending' && <Chip label="wartend" size="small" sx={{
                        height: 16,
                        fontSize: '0.65rem'
                      }}/>}
                      {u.subtitleStatus === 'uploading' && <CircularProgress size={12}/>}
                      {u.subtitleStatus === 'done' &&
                      ...
                      {u.subtitleStatus === 'error' && (
                          <Box sx={{display: 'flex', alignItems: 'center', gap: 0.5}}>
                            ...
                            {u.subtitleError && <Typography variant="caption"
                                                            color="error">{u.subtitleError}</Typography>}
                          </Box>
                      )}
                    </Box>
                )}
