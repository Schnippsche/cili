#!/usr/bin/env python3
"""
Telegram → CILI Erfahrungsberichte-Importer (Tier)
...
Einrichtung:
  ...
  3. .env-Datei anlegen (Vorlage: scripts/telegram_import_tier.env.example)
  4. Beim ersten Start: Telefonnummer + Code eingeben (Session wird danach gespeichert
     und von telegram_import_mensch.py / telegram_import_webinare.py mitgenutzt)

Verwendung:
  python scripts/telegram_import_tier.py
  python scripts/telegram_import_tier.py --from-date 2026-06-01
  python scripts/telegram_import_tier.py --env scripts/telegram_import_tiere.env
"""
...
def main() -> None:
    args = common.parse_args("Telegram → CILI Erfahrungsberichte-Importer (Tier)")
